...
std::optional<std::int64_t> toInt(std::span<const char> digits)
{
    std::int64_t x = -1;
    const auto [ptr_, err] = std::from_chars(digits.data(),
                                             digits.data()+digits.size(), x);
    return (err == std::errc()) ? std::optional(x) : std::nullopt;
}
                                                 ...
                                                 std::int64_t last)
{
    ...
    std::unordered_set<std::int64_t> invalids;
    for (std::int64_t x = 1; ; ++x) {
        char buffer[40];
        const auto [xEnd, err] = std::to_chars(buffer,
                                               buffer+sizeof(buffer), x);
        assert(err == std::errc());
        for (auto ptr = xEnd; ; ) {
            ptr = std::copy(buffer, xEnd, ptr);
            const std::span digits(buffer, ptr);
            const auto num = toInt(digits);
            if (!num || (*num > last)) {
                if (digits.size() == (2UL*(xEnd-buffer))) {
                    std::vector result(invalids.begin(), invalids.end());
            ...
            if ((*num >= first) && !invalids.insert(*num).second)
                break;
        }
    }
}
...
void runTests()
{
    ...
    check(__LINE__, std::array{99L, 111L},   95,         115);
    check(__LINE__, std::array{999L, 1010L}, 998,        1012);
    ...
    check(__LINE__, std::array{565656L},     565653,     565659);
    check(__LINE__, std::array{824824824L},  824824821,  824824827);
    check(__LINE__, std::array{2121212121L}, 2121212118, 2121212124);
    check(
        __LINE__,
        4174379265,
