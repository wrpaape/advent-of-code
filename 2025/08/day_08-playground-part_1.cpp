...
std::vector<std::vector<Point>> connect(std::span<const Point> points,
                                        std::int64_t numConnections)
{
    ...
    for (std::int64_t i = 0; i < n; ++i)
        for (std::int64_t j = i+1; j < n; ++j)
            edges.push_back({dSq(points[i], points[j]), i, j});
    assert(edges.size() >= std::size_t(numConnections));
    ...
    if (numConnections > 0)
        std::nth_element(edges.begin(), edges.begin()+numConnections-1, edges.end());

    for (const auto [_, i, j] : std::span(edges.begin(),
                                          edges.begin()+numConnections))
        join(i, j);
    std::vector<std::vector<Point>> result(n);
    for (std::int64_t i = 0; i < n; ++i)
        result[getC(i)].push_back(points[i]);
    std::erase_if(result, [](const auto& v) { return v.empty(); });
    return result;
}

std::int64_t solve(std::span<const Point> input, std::int64_t numConnections)
{
    assert(!input.empty());

    auto graph = connect(input, numConnections);
    assert(graph.size() >= 3);
    const auto third = graph.begin()+2;
    std::ranges::nth_element(
        graph.begin(), third, graph.end(),
        std::greater{}, [](const auto& circuit) { return circuit.size(); });
    std::int64_t product = 1;
    for (const auto& circuit : std::span(graph.begin(), third+1))
        product *= std::int64_t(circuit.size());
    return product;
}

void check(int lineNumber,
           std::int64_t expectProduct,
           std::string_view source,
           std::int64_t numConnections)
{
    if (const auto product = solve(getInput(source), numConnections);
...
int main()
{
    ...
    const auto total = solve(getInput(), 1000);
